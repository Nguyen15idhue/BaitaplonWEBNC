...
## Auth
- [x] POST /api/auth/register — validate username 3–100 `[a-z0-9._-]`, email regex, pass 8–72
...
## Bookings / Checkouts
- [x] POST /api/bookings (1 transaction Booking+Checkout, chỉ Mock, tour không giá → 422, tour quá EndDate → 400 TOUR_ENDED; header `Idempotency-Key` optional chống trùng đơn)
...
## Users / Audit / Stats
- [x] GET /api/users?search&role&status, PUT /api/users/{id}/role, PUT /api/users/{id}/lock (Admin)
